...
func OpenDB() {
	...
	if err != nil {
		Logger.Error().Msg("DB Connection could not be made")
		return
	}
...
func CloseDB() error {
	// Logger.Info().Msg("DB has been closed.")
	return DB.Close()
}
...
func AddPlayersStats(saveID int, seasonID int, stats []PlayerSeason) error {
	...
	for _, player := range stats {
		if playerID, ok := playersMap[player.UID]; ok {
			...
			Logger.Error().Msg("Player with UID: " + strconv.Itoa(player.UID) + " does not exist in DB.")
		}
	}
	return nil
}
