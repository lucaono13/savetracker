...
onMounted( () => {
    GetAllPlayersTotals().then( (response) => {
        if (response.Error != "") {
        ...
        outGroupedSaves.value = ([...new Set(response.OutTotals.map((item: backend.PlayerTotalsView) => item.saveName))])
        gkGroupedSaves.value = ([...new Set(response.GKTotals.map((item: backend.PlayerTotalsView) => item.saveName))])
    })
})
